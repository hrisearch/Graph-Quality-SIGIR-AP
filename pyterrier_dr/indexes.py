...
class TorchRankedLists:
    def __init__(self, num_results, num_queries, largest=True):
        self.num_results = num_results
        self.num_queries = num_queries
        self.scores = None
        self.docids = None
    ...
    def update(self, scores, docids):
        if self.scores is not None:
            self.scores, idxs = torch.cat([self.scores, scores], dim=1).topk(self.num_results, dim=1, largest=self.largest)
