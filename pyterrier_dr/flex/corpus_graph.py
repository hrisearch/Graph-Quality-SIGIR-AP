...
def _corpus_graph2(self, k=16, batch_size=8192):
    from pyterrier_adaptive import CorpusGraph
    key = ('corpus_graph2', k)
    if key not in self._cache:
        path = self.index_path/f'corpusgraph2_k{k}'
        if not (path/'pt_meta.json').exists():
            candidates = [(p, re.match(r'^corpusgraph2_k([0-9]+)$', str(p).split('/')[-2])) for p in self.index_path.glob('corpusgraph2_k*/pt_meta.json')]
            candidates = sorted([(int(m.group(1)), p) for p, m in candidates if m is not None and int(m.group(1)) > k])
            if candidates:
                self._cache[key] = CorpusGraph.load(candidates[0][1].parent).to_limit_k(k)
            else:
                _build_corpus_graph2(self, k, path, batch_size)
                self._cache[key] = CorpusGraph.load(path)
        else:
            self._cache[key] = CorpusGraph.load(path)
    return self._cache[key]
...
FlexIndex.corpus_graph2 = _corpus_graph2


def _build_corpus_graph2(flex_index, k, out_dir, batch_size):
    S = batch_size
    vectors, meta = flex_index.payload(return_docnos=False)
    num_vecs = vectors.shape[0]
    num_chunks = math.ceil(num_vecs/S)
    rankings = [TorchRankedLists(k, min((i+1)*S, num_vecs) - i*S, largest=False) for i in range(num_chunks)]
    out_dir.mkdir(parents=True, exist_ok=True)
    edges_path = out_dir/'edges.u32.np'
    weights_path = out_dir/'weights.f16.np'
    device = pyterrier_dr.util.infer_device()
