...
class LadrPreemptive(pt.Transformer):
    def __init__(self, flex_index, graph, dense_scorer, j, n='', hops=1, drop_query_vec=False):
        ...
        self.j = j
#        with open('lookups/lookup_' + str(j) + '.npy', 'r') as f:
        self.lookupyadi = np.load('lookups/lookup' + n + '_' + str(j) + '.npy')
    ...
    def transform(self, inp):
        ...
        for qid, df in it:
            ...
            for _ in range(self.hops):
                ...
                #print(docids)
                #print(self.j)
                docids = self.lookupyadi[docids].reshape(-1)
                #print(docids)
                #exit()
                # Lookup End
...
def _pre_ladr(self, k=16, hops=1, j=0, n='', dense_scorer=None, drop_query_vec=False):
    graph = self.corpus_graph(k) if isinstance(k, int) else k # HNSW graph is k
    return LadrPreemptive(self, graph, j=j, n=n, hops=hops, dense_scorer=dense_scorer or self.scorer(), drop_query_vec=drop_query_vec)
FlexIndex.ladr = _pre_ladr # TODO: remove this alias later
FlexIndex.pre_ladr = _pre_ladr

class LadrAdaptive(pt.Transformer):
    def __init__(self, flex_index, graph, dense_scorer, depth=100, max_hops=None, drop_query_vec=False, j=0, n='', strategy='befs'):
        self.flex_index = flex_index
        self.graph = graph
        self.dense_scorer = dense_scorer
        self.depth = depth
    ...
    def getn(self, query_vecs, docids, scores):
        rnd = 0

        if self.strategy == 'befs' or self.strategy == 'a*':
            while self.max_hops is None or rnd < self.max_hops:
                ...
                neighbour_dids = self.lookupyadi[neighbour_dids].reshape(-1)
                ...
                #print(type(neighbour_scores))
                if self.strategy == 'a*':
                    neighbour_scores -= (rnd + 1)
                #print(neighbour_scores)
        ...
        elif self.strategy == 'hc':
            print('kkkkkkk: ' + str(self.max_hops))
            while rnd < self.max_hops:
                dids = docids
#                print(self.graph.edges_data[dids])

                # mat = self.graph.edges_data[dids][:, :2]
                # neighbour_dids = np.unique(mat.reshape(-1))
...
#                exit()
    ...
    def transform(self, inp):
        ...
        for qid, df in it:
            ...
            docids, scores = self.getn(query_vecs, docids, scores)
...
def _ada_ladr(self, k=16, dense_scorer=None, depth=100, max_hops=None, drop_query_vec=False, j=0, n='', strategy='befs'):
    if strategy == 'hc':
        graph = self.corpus_graph(2) if isinstance(k, int) else k
    else:
        graph = self.corpus_graph(k) if isinstance(k, int) else k
    return LadrAdaptive(self, graph, dense_scorer=dense_scorer or self.scorer(), depth=depth, max_hops=max_hops, drop_query_vec=drop_query_vec, j=j, n=n, strategy=strategy)
