...
class FaissRetriever(pt.Indexer):
    def __init__(self, flex_index, faiss_index, n_probe=None, ef_search=None, search_bounded_queue=None, qbatch=64, drop_query_vec=False, j=0, n=''):
...
def _faiss_hnsw_retriever(self, neighbours=32, ef_construction=40, ef_search=16, cache=True, search_bounded_queue=True, qbatch=64, drop_query_vec=False, j=0, n=''):
    pyterrier_dr.util.assert_faiss()
    import faiss
    meta, = self.payload(return_dvecs=False, return_docnos=False)
    ...
    if key not in self._cache:
        ...
        if not os.path.exists(self.index_path/index_name):
            ...
            ### commented
            print(idx.hnsw.efSearch)
            idx.hnsw.efConstruction = ef_construction
            print(idx.hnsw.efConstruction)
            print('----')
            ###
    ...
    return FaissRetriever(self, self._cache[key], ef_search=ef_search, search_bounded_queue=search_bounded_queue, qbatch=qbatch, drop_query_vec=drop_query_vec, j=j, n=n)
